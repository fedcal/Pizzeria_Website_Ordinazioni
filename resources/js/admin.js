                        ...
                        <div class="inline-block relative w-64">
                            <form action="/admin/orders/status" method="POST">
                                ...
                                <select name="status" onchange="this.form.submit()" class="block appearance-none w-full bg-white border border-gray-400 hover:border-gray-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline">
                                    <option value="order_placed" ${ order.status === 'order_placed' ? 'selected' : '' }> Nuovo ordine </option>
                                    <option value="confirmed" ${ order.status === 'confirmed' ? 'selected' : '' }> Confermato </option>
                                    <option value="prepared" ${ order.status === 'prepared' ? 'selected' : '' }> In preparazione </option>
                                    <option value="daRitirare" ${ order.status === 'daRitirare' ? 'selected' : '' }> Pronto per il ritiro </option>
                                    <option value="completed" ${ order.status === 'completed' ? 'selected' : '' }> Completato </option>
                                    <option value="canceled" ${ order.status === 'cancelled' ? 'selected' : '' }> Rifiutare </option>
                                </select>
                            </form>
